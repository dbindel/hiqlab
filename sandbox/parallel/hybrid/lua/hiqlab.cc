...
#include "qpassembly_trilinoslua.h"
#include "qtrilinos_epetralua.h"
#include "qtrilinos_amesoslua.h"
#include "qtrilinos_aztecoolua.h"
#include "qtrilinos_teuchoslua.h"
#include "qtrilinos_epetraextlua.h"
#include "qtrilinos_anasazilua.h"
#include "qtrilinos_arpacklua.h"
#include "qtrilinos_mllua.h"
#include "qtrilinos_ifpacklua.h"
#include "qtrilinos_komplexlua.h"
#include "qtrilinos_meshlua.h"
...
static int hiqlab_defs(lua_State* state)
{
    ...
    tolua_qpassembly_trilinos_open(state);
    tolua_qtrilinos_epetra_open(state);
    tolua_qtrilinos_amesos_open(state);
    tolua_qtrilinos_aztecoo_open(state);
    tolua_qtrilinos_teuchos_open(state);
    tolua_qtrilinos_epetraext_open(state);
    tolua_qtrilinos_anasazi_open(state);
    tolua_qtrilinos_arpack_open(state);
    tolua_qtrilinos_ml_open(state);
    tolua_qtrilinos_ifpack_open(state);
    tolua_qtrilinos_komplex_open(state);
    ...
    tolua_qpetsc_trilinos_open(state);
    tolua_qpetsc_mesh_open(state);
    tolua_qtrilinos_mesh_open(state);
...
static int pmain (lua_State *l) {
  ...
  int status;
  int interactive = 0;
  if (s->argc==2 && s->argv[1] && s->argv[1][0]) progname = s->argv[1];
  L = l;
  lua_userinit(l);  /* open libraries */
  status = handle_luainit();
  if (progname)
      status = file_input(progname);
  if (s->interactive)
      manual_input();
  s->status = status;
  return 0;
...
int main(int argc, char *argv[]) 
{
  ...
  // -- Start MPI from HIQLAB
  MPI_Init(&argc, &argv);

  // -- Construct Comm for TRILINOS
  HIQLAB_Comm = new Epetra_MpiComm(MPI_COMM_WORLD);

  // -- Construct Comm for PETSC
  PetscErrorCode ierr;
  int pargc = 2;
  char* pargv[2] = {"mpirun","hiqlab"};
  static char* help = "Test package";
  char** pargvp;
  pargvp = pargv;

  PetscInitialize(&pargc,&pargvp,(char *)0,help);
#if HAVE_HIQLAB_SLEPC
  SlepcInitialize(&pargc,&pargvp,(char *)0,help);
#endif
