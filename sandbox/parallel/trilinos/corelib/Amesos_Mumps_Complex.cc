...
//=============================================================================
Amesos_Mumps_Complex::Amesos_Mumps_Complex(const Epetra_LinearProblem_Complex &prob ) :
  Problem_(&prob),
  NoDestroy_(false),
  MaxProcs_(-1),
...
//=============================================================================
void Amesos_Mumps_Complex::Destroy()
{
  if (!NoDestroy_)
  {
    ...
    if (Comm().MyPID() < MaxProcs_) zmumps_c(&(MDS));
...
//=============================================================================
Amesos_Mumps_Complex::~Amesos_Mumps_Complex(void)
{
  Destroy();
}

//=============================================================================
int Amesos_Mumps_Complex::ConvertToTriplet(const bool OnlyValues)
{

  Epetra_RowMatrix* ptr;
  Epetra_RowMatrix* ptri;
  if (Comm().NumProc() == 1) {
    ptr = &Matrix();
    ptri= &Matrix_i();
  } else {
    ptr = &RedistrMatrix(true);
    ptri= &RedistrMatrix_i(true);
  }
  ...
  Valr.resize(ptr->NumMyNonzeros());
  Vali.resize(ptr->NumMyNonzeros());
  Valz.resize(ptr->NumMyNonzeros());

  int MaxNumEntries = ptr->MaxNumEntries();
  vector<int> Indices;
  vector<double> Values;
  vector<double> Valuesi;
  Indices.resize(MaxNumEntries);
  Values.resize(MaxNumEntries);
  Valuesi.resize(MaxNumEntries);
  ...
  for (int i = 0; i < ptr->NumMyRows() ; ++i) {
    ...
    // -- Assuming same nonzero pattern for Real/Imag
    ierr = ptr->ExtractMyRowCopy(i, MaxNumEntries,
                                   NumEntries, &Values[0],
                                   &Indices[0]);
    AMESOS_CHK_ERR(ierr);
    ierr = ptri->ExtractMyRowCopy(i, MaxNumEntries,
                                   NumEntries, &Valuesi[0],
                                   &Indices[0]);
    AMESOS_CHK_ERR(ierr);
    ...
    for (int j = 0 ; j < NumEntries ; ++j) {
      ...
      Valr[count] = Values[j];
      Vali[count] = Valuesi[j];
      count++;

    }
  }

  copy_complex(&Valr[0], &Vali[0], &Valz[0], count);
...
void Amesos_Mumps_Complex::SetICNTL(int pos, int value)
{
  ICNTL[pos] = value;
}
...
//=============================================================================
// NOTE: suppose first position is 1 (as in FORTRAN)
void Amesos_Mumps_Complex::SetCNTL(int pos, double value)
{
  CNTL[pos] = value;
}

//=============================================================================
void Amesos_Mumps_Complex::SetICNTLandCNTL()
{
...
//=============================================================================
int Amesos_Mumps_Complex::SetParameters( Teuchos::ParameterList & ParameterList)
{
  // ========================================= //
  ...
  if (ParameterList.isSublist("mumps"))
  {
    ...
    // ordering
     ...
     {
       dcomplex * ColSca = 0;
       ColSca = MumpsParams.get<dcomplex *>("ColScaling");
     ...
     {
       dcomplex* RowSca = 0;
       RowSca = MumpsParams.get<dcomplex *>("RowScaling");
...
void Amesos_Mumps_Complex::CheckParameters()
{
...
//=============================================================================
int Amesos_Mumps_Complex::SymbolicFactorization()
{
  ...
  if (MaxProcs_ != Comm().NumProc())
  {
    ...
    MDS.comm_fortran = (int) MPI_Comm_c2f( MUMPSComm_);
//    MDS.comm_fortran = (F_INT) MPI_Comm_c2f( MUMPSComm_);
  ...
  else
  {
    ...
    assert (MpiComm != 0);
    MDS.comm_fortran = (int) MPI_Comm_c2f(MpiComm->GetMpiComm());
...
//  MDS.sym = MatrixProperty_;
  ...
  if (RowSca_ != 0)
  {
    MDS.rowsca = (mumps_double_complex *) RowSca_;
    MDS.colsca = (mumps_double_complex *) ColSca_;
  }
...
int Amesos_Mumps_Complex::NumericFactorization()
{
  IsNumericFactorizationOK_ = false;
  ...
  {
    if (Comm().MyPID() < MaxProcs_)
      MDS.a_loc = (mumps_double_complex *) &Valz[0];
  }
  else
    MDS.a = (mumps_double_complex *) &Valz[0];
...
int Amesos_Mumps_Complex::Solve()
{
  ...
  if (Comm().NumProc() == 1)
  {
    ...
    for (int j = 0 ; j < NumVectors; j++)
    {
      ...
      dcomplex vecXz[Matrix().NumMyRows()];
      for (int i = 0 ; i < Matrix().NumMyRows() ; ++i) {
        vecXz[i] = dcomplex( (*vecB)[j][i], (*vecBi)[j][i] );
      }
      MDS.rhs = (mumps_double_complex *) &vecXz[0];
//      MDS.rhs = (*vecX)[j];

      zmumps_c(&(MDS)) ;  // Perform solve
      static_cast<void>( CheckError( ) );   // Can hang

      for (int i = 0 ; i < Matrix().NumMyRows() ; ++i) {
        (*vecX)[j][i]  = real(vecXz[i]);
        (*vecXi)[j][i] = imag(vecXz[i]);
      }
  ...
  else
  {
    ...
    Epetra_MultiVector SerialVector_i(SerialMap(),NumVectors);

    ResetTime();
    AMESOS_CHK_ERR(SerialVector.Import(*vecB,SerialImporter(),Insert));
    AMESOS_CHK_ERR(SerialVector_i.Import(*vecBi,SerialImporter(),Insert));
    ...
    for (int j = 0 ; j < NumVectors; j++)
    {
      dcomplex* vecXz;
      if (Comm().MyPID() == 0) {
        vecXz = new dcomplex[Matrix().NumGlobalRows()];
        copy_complex( SerialVector[j], SerialVector_i[j], vecXz, Matrix().NumGlobalRows());
        MDS.rhs = (mumps_double_complex *) vecXz;
//        MDS.rhs = SerialVector[j];
      ...
      if (Comm().MyPID() == 0) {
        copy_complex( vecXz, SerialVector[j], SerialVector_i[j], Matrix().NumGlobalRows());
        delete[] vecXz;
      }
      AddTime("solve");
    }
    ...
    AMESOS_CHK_ERR(vecXi->Export(SerialVector_i,SerialImporter(),Insert));
    AddTime("vector redistribution");
  }
/* -- Temporary disable
...
//=============================================================================
void Amesos_Mumps_Complex::PrintStatus() const
{
...
//=============================================================================
int Amesos_Mumps_Complex::CheckError()
{
...
// ======================================================================
void Amesos_Mumps_Complex::PrintTiming() const
{
...
// ===========================================================================
Epetra_RowMatrix& Amesos_Mumps_Complex::Matrix()
{
...
// ===========================================================================
const Epetra_RowMatrix& Amesos_Mumps_Complex::Matrix() const
{
  Epetra_RowMatrix* Matrix = dynamic_cast<Epetra_RowMatrix*>(Problem_->GetOperator());
  assert (Matrix != 0);
  return(*Matrix);
...
// ===========================================================================
Epetra_RowMatrix& Amesos_Mumps_Complex::Matrix_i()
{
  Epetra_RowMatrix* Matrix = dynamic_cast<Epetra_RowMatrix*>(Problem_->GetOperator_i());
  assert (Matrix != 0);
  return(*Matrix);
}

// ===========================================================================
const Epetra_RowMatrix& Amesos_Mumps_Complex::Matrix_i() const
{
  Epetra_RowMatrix* Matrix = dynamic_cast<Epetra_RowMatrix*>(Problem_->GetOperator_i());
  assert (Matrix != 0);
  return(*Matrix);
...
// ===========================================================================
Epetra_Map& Amesos_Mumps_Complex::RedistrMap()
{
...
// ===========================================================================
Epetra_Import& Amesos_Mumps_Complex::RedistrImporter()
{
  assert (Comm().NumProc() != 1);
...
// ===========================================================================
Epetra_RowMatrix& Amesos_Mumps_Complex::RedistrMatrix(const bool ImportMatrix)
{
...
// ===========================================================================
Epetra_RowMatrix& Amesos_Mumps_Complex::RedistrMatrix_i(const bool ImportMatrix)
{
  if (Comm().NumProc() == 1) return(Matrix_i());

  if (ImportMatrix) RedistrMatrix_i_ = null;

  if (RedistrMatrix_i_ == null)
  {
    RedistrMatrix_i_ = rcp(new Epetra_CrsMatrix(Copy,RedistrMap(),0));
    if (ImportMatrix) {
      int ierr = RedistrMatrix_i_->Import(Matrix_i(),RedistrImporter(),Insert);
      assert (ierr == 0);
      ierr = RedistrMatrix_i_->FillComplete();
      assert (ierr == 0);
    }
  }

  return(*RedistrMatrix_i_);
}

// ===========================================================================
Epetra_Map& Amesos_Mumps_Complex::SerialMap()
{
  if (SerialMap_ == null)
  {
...
// ===========================================================================
Epetra_Import& Amesos_Mumps_Complex::SerialImporter()
{
...
// ===========================================================================
double* Amesos_Mumps_Complex::GetRINFO()
{
  return ( MDS.rinfo);
}

// ===========================================================================
int* Amesos_Mumps_Complex::GetINFO()
{
  return (MDS.info);
}

// ===========================================================================
double* Amesos_Mumps_Complex::GetRINFOG()
{
  return (MDS.rinfog);
}

// ===========================================================================
int* Amesos_Mumps_Complex::GetINFOG()
{
  return (MDS.infog);
}
