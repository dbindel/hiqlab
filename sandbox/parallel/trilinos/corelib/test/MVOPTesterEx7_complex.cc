...
*/
  ...
  Teuchos::RefCountPtr<Anasazi::OutputManager<ST> > MyOM 
    = Teuchos::rcp( new Anasazi::BasicOutputManager<ST>() );
  if (verbose) {
  ...
  for (int i = 0; i < dim; ++i) {
      ...
      for (int j = colptr[i]; j < colptr[i+1]; ++j) {
          if (Map->MyGID(rowind[k]-1)) {
	      InputMatr.InsertGlobalValues( rowind[k]-1, 1, &(dvals[k*2])  , ind);
	      InputMati.InsertGlobalValues( rowind[k]-1, 1, &(dvals[k*2+1]), ind);
	  }
	  ++k;
      }
  }
  ...
  Teuchos::RefCountPtr< Anasazi::EpetraOpComplex > A1
