...
// Changing from here
  ...
  head[0] = 0;
  head[1] = 0;
  for (int i = 0; i < FullMap.NumMyElements(); ++i) {
      std::vector<int> indices;
      if (GID_A(head[0])==i) {
         double val_t[Am.NumMyEntries(head[0])];
         int   id_t [Am.NumMyEntries(head[0])];
         int   nnz_t;
         Am.ExtractMyRowCopy(head[0],Am.NumMyEntries(head[0]),nnz_t,val_t,id_t);
         for (int j = 0; j < nnz_t; ++j) {
              id_t[j] = index_mapping1(Am.RowMap().GID(id_t[j]));
              indices.push_back(id_t[j])
         }
      }
      if (head[0]==i) {
         double val_t[Am.NumMyEntries(head[0])];
