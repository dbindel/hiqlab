...
std::cout << Bm;
  ...
  for (int i = 0; i < Am.RowMap().NumMyElements(); ++i) {
      ...
      int   nnz_t;
      Am.ExtractGlobalRowCopy(Am.RowMap().GID(i),Am.NumMyEntries(i),nnz_t,val_t,id_t);
      for (int j = 0; j < nnz_t; ++j)
          id_t[j] = index_mapping2(id_t[j]);

      double val_f[Am.NumMyEntries(i)];
      int   id_f [Am.NumMyEntries(i)];
      int   nnz_f=0;
      for (int j = 0; j < nnz_t; ++j) {
          if (id_t[j] >= 0) {
              id_f[nnz_f] = id_t[j];
      ...
      C.InsertGlobalValues(GID_A[i],nnz_f,val_f,id_f);
  }
  ...
  for (int i = 0; i < Bm.RowMap().NumMyElements(); ++i) {
      ...
      int   nnz_t;
      Bm.ExtractGlobalRowCopy(Bm.RowMap().GID(i),Bm.NumMyEntries(i),nnz_t,val_t,id_t);
      for (int j = 0; j < nnz_t; ++j)
          id_t[j] = index_mapping2(id_t[j]);
      //FIXME: Cannot insert twice into the same place. Also must insert
      //       first to sum.
      double val_f[Bm.NumMyEntries(i)];
      int   id_f [Bm.NumMyEntries(i)];
      int   nnz_f=0;
      for (int j = 0; j < nnz_t; ++j) {
          if (id_t[j] >= 0) {
              id_f[nnz_f] = id_t[j];
      ...
      C.InsertGlobalValues(GID_B[i],nnz_f,val_f,id_f);
...
*/
  ...
  D.InsertCrsMatrix(&A, &index_mapping1, &index_mapping2);
  D.InsertCrsMatrix(&A, &index_mapping1, &index_mapping1);
  D.InsertCrsMatrix(&B, &index_mapping2, &index_mapping1);
  D.InsertCrsMatrix(&B, &index_mapping2, &index_mapping2);
  D.ConstructMatrix();
