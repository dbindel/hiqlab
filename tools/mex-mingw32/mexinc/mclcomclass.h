...
public:
    ModuleLock()
    {
#if defined (_MCLCOMMAIN_H_) || defined (_MCLXLMAIN_H_)
        RequestGlobalLock();
#else
...
public:
    ...
    bool Feval(HMCRINSTANCE hinst, const char* name, int nlhs, mxArray** plhs, int nrhs, mxArray** prhs)
    {
        HWND hWnd = NULL;
        BOOL bRet = FALSE;
    ...
    void RaiseEvent(OLECHAR* lpwszName, IDispatch* pDisp, DISPPARAMS* pDispParams)
    {
...
public:
    ...
    CMCLModule(IMCLEventMap* pEvents)
    {
        m_cLockCount = 0;
		m_hInstance = NULL;
		m_pObjectMap = NULL;
    ...
    CMCLModule(MCLInitializeInstancePtr pInit, MCLTerminateInstancePtr pTerm)
    {
		m_cLockCount = 0;
		m_hInstance = NULL;
		m_pObjectMap = NULL;
    ...
    CMCLModule(MCLInitializeInstancePtr pInit, MCLTerminateInstancePtr pTerm, IMCLEventMap* pEvents)
    {
		m_cLockCount = 0;
		m_hInstance = NULL;
		m_pObjectMap = NULL;
...
public:
	...
	virtual HRESULT UpdateRegistry(BOOL bRegister)
    {
      ...
      if (bRegister)
      {
        ...
        wDllPath = wcsdup(swDllPath->hPtr);
        deleteWcsStackPointer(swDllPath);
...
--------------------------------------*/
public:
    CMCLSingleModule() : CMCLModule(new mclSingleEventMap()), m_mcrInstance(NULL)
    {
    }
    CMCLSingleModule(MCLInitializeInstancePtr pInit, MCLTerminateInstancePtr pTerm)
        : CMCLModule(pInit, pTerm, new mclSingleEventMap()), m_mcrInstance(NULL)
    {
    }
	...
	static HRESULT __stdcall Error(const char* lpszMessage)
	{
		...
		if (lpszMessage != NULL)
		{
                  ...
                  if(mwMbstowcs(slpwszMessage, lpszMessage) < 0) {
                    ...
                    lpwszMessage = wcsdup(slpwszMessage->hPtr);
                    deleteWcsStackPointer(slpwszMessage);
                  }
		...
		if (lpszSource != NULL)
		{
                  ...
                  if ( mwMbstowcs(slpwszSource, lpszSource) < 0){
                    ...
                    lpwszSource=wcsdup(slpwszSource->hPtr);
                    deleteWcsStackPointer( slpwszSource );
                  }
...
--------------------------------------*/
    ...
    void mclRaiseEventA(const char* lpszName, int nargin, mxArray** prhs)
    {
        ...
        lpwszName=wcsdup(slpwszName->hPtr);
        deleteWcsStackPointer(slpwszName);
