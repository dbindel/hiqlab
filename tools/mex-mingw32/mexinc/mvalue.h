/*
 * @(#)mvalue.h    generated by: makeheader 4.21  Tue Jul 26 18:29:24 2005
 *
...
class EXPORT_CLASS Mrefcount_threadsafe {
    ...
    bool array;
public:    
    Mrefcount_threadsafe(bool iarray = false) : refcount(1), array(iarray) { }
    ~Mrefcount_threadsafe() { if (MathWorks::System::atomic_add(&refcount,0) != 1) throw "bad reference count"; }
    void copy() { MathWorks::System::atomic_inc(&refcount); }
    bool destroy() { return MathWorks::System::atomic_dec(&refcount) == 0; }
    bool isarray() { return array; }
    void dump() const { printf ("Refcount == %d (arr=%s)\n", MathWorks::System::atomic_add(const_cast<int32_t*>(&refcount),0), array ? "True" : "False" ); }
    unsigned int count() { return (unsigned int)MathWorks::System::atomic_add(&refcount,0); }
};
...
private:
    ...
    bool islonely() const {
        ...
        return (mxGetReferenceCount(*(extract_data_pointer<mxArray *>(v.slow.d))) == 0);
    }
    int get_type() const {
