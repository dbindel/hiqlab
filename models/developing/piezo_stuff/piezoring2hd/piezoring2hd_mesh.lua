...
issym    = issym     or    0             -- Whether symmetric anchors
dkRi     = dkRi      or   90e-6
...
-- Number of divisions
ndiv_Rim =  8     -- Through thickness division Rim
ndiv_Rom =  8    -- Through thickness division Rom
ndiv_Rp  = 32    -- Divisions along quarter arc
ndiv_bmW =  3    -- Through bmWidth   division 
ndiv_bmL = 10    -- Alogn bmLength division
...
dim_scales    = pz_nondim(beam_material, dkWidth)
...
mesh:set_bc(function(x,y)
  ...
  if meshleq(x, xa3) then return '  u',       0; end

  if mesheq( x, xa6) then return 'uuu', 0, 0, 0; end
  if meshgeq(x, xa4) and (mesheq( y, ya1) or mesheq(y, ya6))
                     then return 'uuu', 0, 0, 0; end
  if meshgeq(x, xa4) then return '  u',       0; end

  if meshbetween(x, xa3, xa4) then return '  u',       1; end
end)
