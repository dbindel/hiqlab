...
//                            v = lua_tonumber(L, t++);
    ...
    // Insert the 1 for the dual of Lagrange Multiplier
    if (mode == 0) { // Find number of potential dofs
        ++nbranch;
    } else if (mode == 1) { // Assignids
        ++nbranch;
    } else if (mode == 2) { // Get R
        ++nbranch;
        double s = mesh->branchu(0,      eltid);
        double l = mesh->branchu(nbranch,eltid);

        mesh->branchf(0,eltid)       += -l;
        mesh->branchf(nbranch,eltid) +=  s;

    } else if (mode == 3) { // Get K

        ++nbranch;
        int id[2];
        id[0] = mesh->ibranch(0      ,eltid);
        id[1] = mesh->ibranch(nbranch,eltid);

        double Ke[4] = { 0, cx,-cx, 0 };
