    ...
    PMLElement(3), rho(rho), at(at), cp(cp), kt(kt), T0(T0)
{
    ...
    // -- Axisymmetric isotropic
    thermoelasticity_axis(Db,lambda,mu,at);
    split_Db(Db);
}
...
void ME::split_Db(double* Db)
{
    /* <generator matexpr>
      input Db(5,5);
      Pm = [eye(4); 0,0,0,0];  // Just mechanical components
      Pt = [0; 0; 0; 0; 1];    // Just thermal component
      output D(4,4) = Pm'*Db*Pm;  // Purely mechanical
      output vp(4)  = Pm'*Db*Pt;  // Coupling
    */
}
    ...
    for (int j = 0; j < nen; ++j) {
        for (int i = 0; i < nen; ++i) {
            ...
            /* <generator matexpr complex="T">
              input D symmetric(4), vp(4), kt, Ni, Nj, r;
...
template<class T> inline
void ME::add_C(T* dNdx, double* N, double r, T W, QMatrix<T>& K)
{
    ...
    for (int j = 0; j < nen; ++j) {
        for (int i = 0; i < nen; ++i) {
            ...
            /* <generator matexpr complex="T">
              input D symmetric(4), vp(4), Ni, Nj, r, Ctherm, T0;
              complex input dNj(2), W;
              complex inout Ktx_ij[nen3](1,3);
              Ktx_ij += [T0*(Ni*vp'*Bmatrix(dNj,Nj,r)), Ctherm*Ni*Nj] * W;
            */
        }
    }
}
...
void ME::add_Ku(T* dNdx, double* N, double r, T* u, T W, QMatrix<T>& F)
{
    ...
    for (int j = 0; j < nen; ++j) {
        ...
        double Nj  = N[j];
        /* <generator matexpr complex="T">
          input D symmetric(4), vp(4), kt, Nj, r;
          complex input utj(3), dNj(2);
          complex inout stress(4)    += [D*Bmatrix(dNj, Nj, r), -vp*Nj]*utj;
    ...
    for (int i = 0; i < nen; ++i) {
        ...
        /* <generator matexpr complex="T">
          input Ni, r;
          complex input dNi(2), stress(4), heat_flux(2), W;
          complex inout Fi(3) += [Bmatrix(dNi, Ni, r)'*stress; 
                                  dNi'*heat_flux] * W;
        */
    }
}
...
template<class T> inline
void ME::add_Cv(T* dNdx, double* N, double r, T* v, T W, QMatrix<T>& F)
{
    ...
    for (int j = 0; j < nen; ++j) {
        ...
        T* utj = v+3*j;
        /* <generator matexpr complex="T">
          input vp(4), T0, Ctherm, Nj, r;
          complex input utj(3), dNj(2);
          complex inout thetad += [T0*(vp'*Bmatrix(dNj,Nj,r)), Ctherm*Nj]*utj;
        */
    }
    for (int i = 0; i < nen; ++i)
        F(2,i) += N[i] * thetad * W;
                     ...
                     double cx, double cv, double ca)
{
    ...
    if (has_stretch()) {
       ...
       for (Quad2dInt iter(nen); !iter.end(); iter.next()) {
           ...
           if (cv)  add_C(pshape.dNdxt(), pshape.N(), pshape.x(0), cv*Wt, Ke);
       }
       K->add(id, 3*nen, Ke.data);
        ...
        for (Quad2dInt iter(nen); !iter.end(); iter.next()) {
            ...
            if (cv)  add_C(shape.dNdx(), shape.N(), shape.x(0), cv*Wt, Ke);
        }
        K->add(id, 3*nen, Ke.data);
    }
}
...
void ME::assemble_R(Mesh* mesh, int eltid)
{
    ...
    if (has_stretch()) {
       ...
       for (Quad2dInt iter(nen); !iter.end(); iter.next()) {
           ...
           add_Cv(pshape.dNdxt(), pshape.N(), pshape.x(0), nodev, Wt, Fe);
       }
       add_local_f(mesh, eltid, Fe.data);
        ...
        for (Quad2dInt iter(nen); !iter.end(); iter.next()) {
            ...
            add_Cv(shape.dNdx(), shape.N(), shape.x(0), nodev, Wt, Fe);
        }
        add_local_f(mesh, eltid, Fe.data);
    }
}
